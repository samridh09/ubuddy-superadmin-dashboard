...
import {
  ...
  Th, Td, Tr, EmptyRow, AssignedStaffDisplay, SectionPill,
} from './ui';
import { StudentPicViewProps } from '@/types';

export const StudentPicView: React.FC<StudentPicViewProps> = ({
  query, updateFilters, isAssigning, setIsAssigning, handleAssignClick,
  filteredData, handleSort, SortIcon, staffList, updateStaffMock
}) => {
  const handleToggle = () => {
  ...
  return (
    <PageWrapper>
      {/* Header */}
      <PageHeader
        title="Staff Permissions | Student Pic"
        actions={
          isAssigning ? (
            <>
