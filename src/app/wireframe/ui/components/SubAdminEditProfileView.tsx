...
import { User, Check, X, Upload, Edit2, Mail, Phone, Calendar, Info, History } from 'lucide-react';
...
import { PROFILE_INPUT_VIEW as INPUT_VIEW, PROFILE_INPUT_EDIT as INPUT_EDIT, PROFILE_INPUT_ERROR as INPUT_ERROR } from './styles';
...
export const SubAdminEditProfileView: React.FC<{ data: SubAdminEditData; adminId: string }> = ({ data, adminId }) => {
  const router   = useRouter();
  const idParam  = `?adminId=${adminId}`;
  const [saveErr, setSaveErr]       = useState('');
  const [errors, setErrors] = useState<Record<string, string>>({});
  const [activeField, setActiveField] = useState<string | null>(null);
  const [draftData, setDraftData] = useState<SubAdminEditData>(data);
  ...
  useEffect(() => { setDraftData(data); }, [data]);
  ...
  const handlePhotoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const { data: json } = await apiClient.put(
        API_ENDPOINTS.schoolAdmins.byId(schoolId, adminId), fd,
        { headers: { 'Content-Type': 'multipart/form-data' } },
      );
      if (!json.success) setPreviewUrl(data.avatar || null);
    } catch { setPreviewUrl(data.avatar || null); }
  };
  ...
  const handleGlobalSave = async () => {
    ...
    const schoolId = localStorage.getItem('schoolId');
    if (!schoolId) { setSaveErr('School ID missing. Please log in again.'); return; }
    ...
    try {
      ...
      // Successfully saved
      router.push(`/dashboard/schooladmin/sub-admin/profile${idParam}`);
      router.refresh();
    } catch (err: any) {
  ...
  return (
    <PageWrapper>
      <PageHeader title="Edit Sub-Admin" subtitle={`${data.name} • ${data.designation}`} showBack />
